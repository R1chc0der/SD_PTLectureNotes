/* 
    * Let's build a  dice game!
    - User plays the dice game, roll dice up to 3 times, and I wan tto focus on scoring per dice roll and totaling the score at the end of the game.
...
// Make a function to update our score as well as print a message (console log) to give up feedback
function scoreDice(dict, dieSide) {
    // Create some math ot add new roll score value to our base score variable
    score += dict[dieSide]
...
// Build a function that will generate a random number limited by 6 (used a six side dice) and increase our roll count variable numberOfRolls (reassignment)
// Just picks with side we land on and adds to roll count #
function rollDie() {
    // Generate a random # that i sleep than 6
    ...
    if(validDieState.includes(newDieState)){
        ...
        if (stateOfDie == "roll") {
            ...
            if (numberOfRolls === 3){
                ...
                rollDie();// will get a random value and increase # of rolls

                // Write message
                console.log(`You rolled a ${diceRollValue}! World you like to roll again or finalize the score?`);

                // score the rolled dice and print a message of rolls made?
                scoreDice(scoring, dieRollValue); // Run function build above
...
//Call on the playState function and give die action as argument
// function roundTwo() {
