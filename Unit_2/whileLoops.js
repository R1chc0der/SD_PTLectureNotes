...
let city = [
  ...
  while (structure != "empty street"){
    console.log(`Spider-Man swings form the ${structure}!`);
    pos++
    structure = city[pos];
  }
  ...
  /* 
    ? Do While Loop: also loops through a block of code while a specified condition is true  BUT...
        ...
        - Works just like while loops with the exception that it will run a code block at least once.
...
// Mock switching account from deactivated to activated, use a boolean
...
do {
    ...
    console.log(
        `acct${count} is currently inactive: set to ${accounts[`acct${count}`]}`
      );
    ...
    console.log(
        `acct${count} is now active: account is set to ${accounts[`acct${count}`]}`
      );
...
//Finish message
console.log("All account are now active.");
