/* 
    ...
    - Wonderful way to access data we are trying to obtain
        ...
        - Ex: data stored in database, cloud storage, or some else's public database(via a public API)
...
const log = console.log; // creating a shortcut variable to hold the console.log functionality
...
fetch(testEndpoint)
    ...
    .then((res) => {
        ...
        return res.json(); // json-ification our response into something we can use adn returning it
    })
    .then((data) =>{
        log("3rd fetch .then", data); // logging the jason object (now we can use it!)
        seeDogPic(data);
    });

     /* 
        ? JSON(what we used about^)
    ...
    */
    ...
    function seeDogPic(jsonDataFromFetch){
        log("In the dog pic function", jsonDataFromFetch);
       
        //  Create element
        
        let img = document.createElement("img");
        
        // Assigning img attributes
        img.src = jsonDataFromFetch.message;


        //Append img attributes
...
function fetchFellowship(){
    ...
    .then(res => res.json())
    .then(data => {
        let members = data.fetchFellowship; 
        log(members);
...
// Async/Await
async function getCatFat3(){
    try{
        //let response = fetch(url); // break - used to show "catch"
